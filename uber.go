...
const (
	...
	RequestEstimateEndpoint = "requests/estimate"
	RequestUpdateEndpoint   = "sandbox/requests"
	ProductEndpoint         = "products"
	ProductUpdateEndpoint   = "sandbox/products"
...
// PriceEstimate contains the information relating to a price estimate request for an Uber done on behalf of a
// user.
type PriceEstimate struct {
	SurgeConfirmationHref string  `json:"surge_confirmation_href"`
	SurgeMultiplier       float64 `json:"surge_multiplier"`
	HighEstimate          int     `json:"high_estimate"`
	LowEstimate           int     `json:"low_estimate"`
	Minimum               int     `json:"mininum"`
	Display               string  `json:"display"`
	CurrencyCode          string  `json:"currency_code"`
}

// TripEstimate contains the information relating to a trip estimate request for an Uber done on behalf of a
// user.
...
// Estimate contains the information relating to a estimate request for an Uber done on behalf of a
// user.
type Estimate struct {
	PickupEstimate int           `json:"pickup_estimate"`
	Price          PriceEstimate `json:"price"`
	Trip           TripEstimate  `json:"trip"`
}
