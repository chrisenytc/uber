...
// PostRequestEstimate allows a ride estimate to be requested on behalf of an Uber user given
// their desired product, start, and end locations.
func (c *Client) PostRequestEstimate(
	productID string, startLat, startLon, endLat, endLon float64, startPlaceID string, endPlaceID string,
) (*Estimate, error) {
	payload := requestReq{
		productID:      productID,
		startLatitude:  startLat,
	...
	request := new(requestEstimateResp)

	if err := c.httpReqDo("POST", RequestEstimateEndpoint, payload, true, request); err != nil {
		return nil, err
	}
...
// PutRequest allows a ride status to be updated on behalf of an Uber user given
// their desired product, start, and end locations.
func (c *Client) PutRequest(requestID, status string) (*Request, error) {
	payload := requestUpdateReq{
		status: status,
	}
	request := new(requestResp)

	if err := c.httpReqDo("PUT", fmt.Sprintf("%s/%s", RequestUpdateEndpoint, requestID), payload, true, request); err != nil {
		return nil, err
	}
...
// PutRequest allows a ride status to be updated on behalf of an Uber user given
// their desired product, start, and end locations.
func (c *Client) PutProduct(productID string, surgeMultiplier float64, driversAvailable bool) (*Product, error) {
	payload := productReq{
		surgeMultiplier:  surgeMultiplier,
		driversAvailable: driversAvailable,
	}
	request := new(productResp)

	if err := c.httpReqDo("PUT", fmt.Sprintf("%s/%s", ProductUpdateEndpoint, productID), payload, true, request); err != nil {
		return nil, err
	}

	return &request.Product, nil
}
